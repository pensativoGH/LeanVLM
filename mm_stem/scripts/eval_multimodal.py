#!/usr/bin/env python3
"""Unified evaluation script for MMMU, MMBench, and MathVista."""
...
def load_mmmu_dataset(data_dir: str):
    """Load MMMU validation set from all subjects."""
    samples = []
            ...
            if "validation" in ds:
                for item in ds["validation"]:
                    ...
                    if image is not None:
                        choices = [item.get(f"option_{i}", "") for i in range(ord('a'), ord('e'))]  # a,b,c,d
                        choices = [c for c in choices if c]  # filter empty
...
def evaluate_dataset(
    ...
    for batch_start in pbar:
        ...
        for sample in batch_samples:
            ...
            prompts.append(format_prompt(processor.tokenizer, q))
        ...
        for i, sample in enumerate(batch_samples):
            ...
            if sample.get("choices"):
                ...
                # Free-form: check if answer is in response
